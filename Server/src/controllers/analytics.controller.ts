...
/**
 * Normalize a journal ML score to the wellness scale [1, 9].
 * The ML model returns a confidence value (0–1 or 0–100) and an emotion type.
 * - "positive" emotionType → high confidence = higher wellness.
 * - "negative" emotionType → high confidence = lower wellness (inverted).
 */
...
export async function getMoodTrends(req: AuthRequest, res: Response) {
  try {
    ...
    const data = await getCachedOrCompute(req.userId, cacheKey, 0, async () => {
      ...
      // Daily aggregated journal ML scores
